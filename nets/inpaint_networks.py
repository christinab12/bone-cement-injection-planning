...
from nets.inpaint_tools import extract_image_patches, flow_to_image, reduce_mean, reduce_sum, same_padding
...
class Generator(nn.Module):
    ...
    def forward(self, x, seg, mask):
        x_stage1 = self.coarse_generator(x, seg, mask)
        x_stage2_mask, x_stage2_ct, offset_flow = self.fine_generator(x, x_stage1, seg, mask) 
        return x_stage1, x_stage2_mask, x_stage2_ct
...
class CoarseGenerator(nn.Module):
    def __init__(self, input_dim, cnum, use_cuda):
        ...
        self.conv17 = gen_conv(cnum//2, 1, 3, 1, 1, activation='none')
    ...
    def forward(self, x, seg, mask):
        ...
        if self.use_cuda:
            ones = ones.cuda()
        x = self.conv1(torch.cat([x, seg, ones, mask], dim=1))
...
class FineGenerator(nn.Module):
    def __init__(self, input_dim, cnum, use_cuda):
        ...
        self.contextul_attention = ContextualAttention(self.use_cuda, ksize=3, stride=1, rate=2, fuse_k=3, softmax_scale=10,fuse=True)
        ...
        self.allconv17 = gen_conv(cnum//2, 6, 3, 1, 1, activation='none')
        self.allconv18 = gen_conv(cnum//2, 1, 3, 1, 1, activation='none')

    def forward(self, xin, x_stage1, seg, mask):
        x1_inpaint = x_stage1 * mask + seg * (1. - mask)
        # For indicating the boundaries of images
        ...
        # conv branch
        xnow = torch.cat([x1_inpaint, seg, ones, mask], dim=1)  # 
        x = self.conv1(xnow)
        x = self.conv2_downsample(x)
        x = self.conv3(x)
        x = self.conv4_downsample(x)
        ...
        x_mask = self.allconv17(x)
        x_ct = self.allconv18(x)
        
        x_stage2_mask = torch.clamp(x_mask, -1., 1.)
        x_stage2_ct = torch.clamp(x_ct, -1., 1.)
        
        return x_stage2_mask, x_stage2_ct, offset_flow
